...
switch ($action) {
	default :
		control('app\\controllers', 'CalcKredytCtrl','generateView', ['user','admin']);
	case 'login': 
		control('app\\controllers', 'CalcKredytLoginCtrl','doLogin');
	case 'calcCompute' : 
		//zamiast pierwszego parametru można podać null lub '' wtedy zostanie przyjęta domyślna przestrzeń nazw dla kontrolerów
		control(null, 'CalcKredytCtrl',	'process',['user','admin']);
 	case 'chroniona' : 
		//zamiast pierwszego parametru można podać null lub '' wtedy zostanie przyjęta domyślna przestrzeń nazw dla kontrolerów
		control(null, 'CalcKredytChronionaCtrl','process',['user','admin']);           
	case 'logout' : 
		control(null, 'CalcKredytLoginCtrl','doLogout',	['user','admin']);
}
